// Glaze Lab recipe data.
// - PROTEINS: the swappable protein axis (salmon, chicken, beef, pork, shrimp,
//   tofu). Each carries its air-fryer temp/time/target-internal + a prep tip;
//   steak and salmon also carry doneness `levels`.
// - PANTRY: the toggleable ingredient master list (proteins included, so they
//   gate dishes too). A dish greys out when the cook is out of anything it needs.
// - STAPLES: always-on basics (salt, oil, water...) that never gate a dish.
// - GLAZE: 10 air-fryer glaze *builds* — protein-agnostic; the picked protein is
//   injected at render (see lib/protein.js) with its temp/time/doneness.
// - RICE: 27 rice-cooker dishes (incl. grains). Protein-forward ones set
//   `usesProtein`; veg/plain ones are fixed. Each shows its COSORI `cooker`.
// - SIDES: 8 fixed sides — air-fryer (own temp/time) and steamed (Steam function).
// A build ingredient's `item` must be a PANTRY id or a STAPLES id
// (enforced by recipes.test.js).

export const STAPLES = new Set(['salt', 'pepper', 'water', 'oil', 'sugar', 'butter', 'egg'])

// Air-fryer temp/time/doneness per protein, from the appliance research
// (Innsky 5.8-qt basket; USDA safe internal temps). Times are starting points —
// the app tells you to verify internal temp with a thermometer, because set
// temps drift. Doneness is the target internal temperature.
export const PROTEINS = [
  // fmt: id, label, amount/unit, air-fryer tempF, cookSeconds, target internal, prep tip
  {
    id: 'salmon',
    label: 'Salmon',
    amount: 2,
    unit: 'fillets',
    tempF: 400,
    cookSeconds: 480,
    doneness: '125–130°F (medium); 145°F well',
    tip: 'Pat dry; skin-side down; no foil; single layer',
    levels: [
      {
        id: 'medium',
        label: 'Medium',
        cookSeconds: 480,
        doneness: '125–130°F medium',
        default: true,
      },
      { id: 'well', label: 'Well', cookSeconds: 600, doneness: '145°F well (USDA)' },
    ],
  },
  {
    id: 'chickenthigh',
    label: 'Chicken thigh',
    amount: 2,
    unit: 'thighs',
    tempF: 380,
    cookSeconds: 1440,
    doneness: '165°F min; 175–185°F best',
    tip: 'Skin-side down first; pat dry',
  },
  {
    id: 'chickenbreast',
    label: 'Chicken breast',
    amount: 1,
    unit: 'breast',
    tempF: 375,
    cookSeconds: 960,
    doneness: '165°F (pull at 160°F)',
    tip: 'Brine 30 min; pat dry; even thickness',
  },
  {
    id: 'shrimp',
    label: 'Shrimp',
    amount: 200,
    unit: 'g',
    tempF: 400,
    cookSeconds: 360,
    doneness: 'opaque & pearly',
    tip: 'Large/jumbo best; single layer; do not overcook',
  },
  {
    id: 'tofu',
    label: 'Firm tofu',
    amount: 200,
    unit: 'g',
    tempF: 375,
    cookSeconds: 1080,
    doneness: 'golden & crisp',
    tip: 'Press 15 min; ¾-inch cubes; cornstarch for crunch',
  },
  {
    id: 'steak',
    label: 'Steak',
    amount: 2,
    unit: 'steaks',
    tempF: 400,
    cookSeconds: 480,
    doneness: '130–135°F med-rare; 145°F USDA',
    tip: 'Pat dry; flip once; rest 5 min',
    levels: [
      { id: 'rare', label: 'Rare', cookSeconds: 420, doneness: '120–125°F rare' },
      {
        id: 'medrare',
        label: 'Med-rare',
        cookSeconds: 480,
        doneness: '130–135°F med-rare',
        default: true,
      },
      { id: 'medium', label: 'Medium', cookSeconds: 540, doneness: '140–145°F medium' },
      { id: 'well', label: 'Well', cookSeconds: 660, doneness: '155°F+ well' },
    ],
  },
  {
    id: 'porkchop',
    label: 'Pork chop',
    amount: 2,
    unit: 'chops',
    tempF: 400,
    cookSeconds: 780,
    doneness: '145°F + 3-min rest',
    tip: 'Pat dry; flip halfway; rest 3 min',
  },
  {
    id: 'porktenderloin',
    label: 'Pork tenderloin',
    amount: 1,
    unit: 'tenderloin',
    tempF: 400,
    cookSeconds: 1200,
    doneness: '145°F + rest',
    tip: 'Turn halfway; rest before slicing',
  },
]

export const PANTRY = [
  // Glaze & sauce
  { id: 'miso', label: 'White miso', group: 'Glaze & Sauce' },
  { id: 'soy', label: 'Soy sauce', group: 'Glaze & Sauce' },
  { id: 'mirin', label: 'Mirin', group: 'Glaze & Sauce' },
  { id: 'honey', label: 'Honey', group: 'Glaze & Sauce' },
  { id: 'maple', label: 'Maple syrup', group: 'Glaze & Sauce' },
  { id: 'gochujang', label: 'Gochujang', group: 'Glaze & Sauce' },
  { id: 'sriracha', label: 'Sriracha', group: 'Glaze & Sauce' },
  { id: 'harissa', label: 'Harissa', group: 'Glaze & Sauce' },
  { id: 'dijon', label: 'Dijon mustard', group: 'Glaze & Sauce' },
  { id: 'ricevinegar', label: 'Rice vinegar', group: 'Glaze & Sauce' },
  { id: 'sesameoil', label: 'Toasted sesame oil', group: 'Glaze & Sauce' },
  { id: 'coconutmilk', label: 'Coconut milk', group: 'Glaze & Sauce' },
  { id: 'currypaste', label: 'Green curry paste', group: 'Glaze & Sauce' },
  { id: 'peanut', label: 'Peanut butter', group: 'Glaze & Sauce' },
  { id: 'brownsugar', label: 'Brown sugar', group: 'Glaze & Sauce' },
  // Aromatics & herbs
  { id: 'garlic', label: 'Garlic', group: 'Aromatics' },
  { id: 'ginger', label: 'Fresh ginger', group: 'Aromatics' },
  { id: 'scallion', label: 'Scallions', group: 'Aromatics' },
  { id: 'cilantro', label: 'Cilantro', group: 'Aromatics' },
  { id: 'basil', label: 'Thai basil', group: 'Aromatics' },
  { id: 'turmeric', label: 'Turmeric', group: 'Aromatics' },
  // Citrus
  { id: 'lemon', label: 'Lemon', group: 'Citrus' },
  { id: 'lime', label: 'Lime', group: 'Citrus' },
  { id: 'orange', label: 'Orange', group: 'Citrus' },
  // Produce
  { id: 'spinach', label: 'Baby spinach', group: 'Produce' },
  { id: 'carrot', label: 'Carrot', group: 'Produce' },
  { id: 'peas', label: 'Green peas', group: 'Produce' },
  { id: 'edamame', label: 'Edamame', group: 'Produce' },
  { id: 'mushroom', label: 'Shiitake mushrooms', group: 'Produce' },
  { id: 'bellpepper', label: 'Bell pepper', group: 'Produce' },
  { id: 'kimchi', label: 'Kimchi', group: 'Produce' },
  { id: 'broccoli', label: 'Broccoli', group: 'Produce' },
  { id: 'cauliflower', label: 'Cauliflower', group: 'Produce' },
  { id: 'brusselsprouts', label: 'Brussels sprouts', group: 'Produce' },
  { id: 'asparagus', label: 'Asparagus', group: 'Produce' },
  { id: 'greenbeans', label: 'Green beans', group: 'Produce' },
  { id: 'potato', label: 'Baby potatoes', group: 'Produce' },
  { id: 'fries', label: 'Frozen fries', group: 'Produce' },
  // Protein
  { id: 'salmon', label: 'Salmon', group: 'Protein' },
  { id: 'chickenthigh', label: 'Chicken thigh', group: 'Protein' },
  { id: 'chickenbreast', label: 'Chicken breast', group: 'Protein' },
  { id: 'steak', label: 'Steak', group: 'Protein' },
  { id: 'porkchop', label: 'Pork chop', group: 'Protein' },
  { id: 'porktenderloin', label: 'Pork tenderloin', group: 'Protein' },
  { id: 'shrimp', label: 'Shrimp', group: 'Protein' },
  { id: 'tofu', label: 'Firm tofu', group: 'Protein' },
  { id: 'wings', label: 'Chicken wings', group: 'Protein' },
  { id: 'chickpea', label: 'Chickpeas', group: 'Protein' },
  { id: 'lentil', label: 'Red lentils', group: 'Protein' },
  // Grain & finish
  { id: 'rice', label: 'Short-grain rice', group: 'Grain' },
  { id: 'jasmine', label: 'Jasmine rice', group: 'Grain' },
  { id: 'quinoa', label: 'Quinoa', group: 'Grain' },
  { id: 'farro', label: 'Farro', group: 'Grain' },
  { id: 'oats', label: 'Steel-cut oats', group: 'Grain' },
  { id: 'stock', label: 'Chicken stock', group: 'Grain' },
  { id: 'furikake', label: 'Furikake', group: 'Finish' },
  { id: 'sesameseed', label: 'Sesame seeds', group: 'Finish' },
]

// Air-fryer glaze builds — protein-agnostic. The protein (and its cook time) is
// injected at render, so steps reference "the protein" and lean on the timer
// rather than hard-coding minutes.
export const GLAZE = [
  {
    id: 'g-miso-maple',
    name: 'Miso–Maple Glaze',
    mode: 'air-fryer',
    lane: 'Sweet & Sticky',
    usesProtein: true,
    blurb: 'Deeply savoury miso rounded out with maple. The house build.',
    ingredients: [
      { item: 'miso', amount: 2, unit: 'tbsp' },
      { item: 'maple', amount: 1, unit: 'tbsp' },
      { item: 'mirin', amount: 1, unit: 'tbsp' },
      { item: 'soy', amount: 1, unit: 'tsp' },
    ],
    steps: [
      'Pat the protein dry and season lightly with salt.',
      'Whisk miso, maple, mirin and soy into a glaze.',
      'Brush half over the protein and air-fry at the shown temp — use the timer.',
      'Brush with the rest and rest 2 minutes before serving.',
    ],
  },
  {
    id: 'g-honey-sriracha',
    name: 'Honey–Sriracha',
    mode: 'air-fryer',
    lane: 'Spicy',
    usesProtein: true,
    blurb: 'Sweet heat with a garlicky edge and a lacquered finish.',
    ingredients: [
      { item: 'honey', amount: 2, unit: 'tbsp' },
      { item: 'sriracha', amount: 1, unit: 'tbsp' },
      { item: 'soy', amount: 1, unit: 'tsp' },
      { item: 'garlic', amount: 1, unit: 'clove' },
    ],
    steps: [
      'Mince garlic and whisk with honey, sriracha and soy.',
      'Coat the protein and air-fry at the shown temp — use the timer.',
      'Spoon over pan glaze and finish under a minute of high heat.',
    ],
  },
  {
    id: 'g-gochujang-lime',
    name: 'Gochujang–Lime',
    mode: 'air-fryer',
    lane: 'Spicy',
    usesProtein: true,
    blurb: 'Funky Korean chili paste brightened with fresh lime.',
    ingredients: [
      { item: 'gochujang', amount: 1.5, unit: 'tbsp' },
      { item: 'honey', amount: 1, unit: 'tbsp' },
      { item: 'lime', amount: 0.5, unit: 'each' },
      { item: 'sesameoil', amount: 1, unit: 'tsp' },
    ],
    steps: [
      'Whisk gochujang, honey, lime juice and sesame oil.',
      'Glaze the protein and air-fry at the shown temp — use the timer.',
      'Zest lime over the top before serving.',
    ],
  },
  {
    id: 'g-teriyaki-ginger',
    name: 'Teriyaki–Ginger',
    mode: 'air-fryer',
    lane: 'Umami',
    usesProtein: true,
    blurb: 'Classic soy–mirin teriyaki with a hit of fresh ginger.',
    ingredients: [
      { item: 'soy', amount: 2, unit: 'tbsp' },
      { item: 'mirin', amount: 1, unit: 'tbsp' },
      { item: 'brownsugar', amount: 1, unit: 'tbsp' },
      { item: 'ginger', amount: 1, unit: 'tsp' },
    ],
    steps: [
      'Simmer soy, mirin, brown sugar and grated ginger 2 minutes to thicken.',
      'Brush the protein and air-fry at the shown temp — use the timer.',
      'Finish with the reduced teriyaki.',
    ],
  },
  {
    id: 'g-dijon-maple',
    name: 'Dijon–Maple',
    mode: 'air-fryer',
    lane: 'Sweet & Sticky',
    usesProtein: true,
    blurb: 'Sharp mustard and maple — a five-minute mix-and-go.',
    ingredients: [
      { item: 'dijon', amount: 1.5, unit: 'tbsp' },
      { item: 'maple', amount: 1.5, unit: 'tbsp' },
      { item: 'lemon', amount: 0.5, unit: 'each' },
    ],
    steps: [
      'Whisk Dijon, maple and lemon juice.',
      'Coat the protein and air-fry at the shown temp until glossy — use the timer.',
    ],
  },
  {
    id: 'g-orange-soy',
    name: 'Orange–Soy',
    mode: 'air-fryer',
    lane: 'Citrus',
    usesProtein: true,
    blurb: 'Bright citrus soy that caramelises at the edges.',
    ingredients: [
      { item: 'orange', amount: 1, unit: 'each' },
      { item: 'soy', amount: 1, unit: 'tbsp' },
      { item: 'honey', amount: 1, unit: 'tbsp' },
      { item: 'ginger', amount: 1, unit: 'tsp' },
    ],
    steps: [
      'Reduce orange juice with soy, honey and ginger until syrupy.',
      'Glaze the protein and air-fry at the shown temp — use the timer.',
      'Brush again and add a little zest.',
    ],
  },
  {
    id: 'g-lemon-garlic-butter',
    name: 'Lemon–Garlic Butter',
    mode: 'air-fryer',
    lane: 'Citrus',
    usesProtein: true,
    blurb: 'Buttery, garlicky, and sharp with lemon. Always works.',
    ingredients: [
      { item: 'butter', amount: 1, unit: 'tbsp' },
      { item: 'garlic', amount: 2, unit: 'cloves' },
      { item: 'lemon', amount: 0.5, unit: 'each' },
      { item: 'dijon', amount: 1, unit: 'tsp' },
    ],
    steps: [
      'Melt butter with minced garlic, lemon juice and Dijon.',
      'Spoon over the protein and air-fry at the shown temp — use the timer.',
      'Baste with pan butter before serving.',
    ],
  },
  {
    id: 'g-miso-furikake',
    name: 'Miso–Furikake Crunch',
    mode: 'air-fryer',
    lane: 'Umami',
    usesProtein: true,
    blurb: 'Miso glaze finished with a seaweed-sesame furikake crust.',
    ingredients: [
      { item: 'miso', amount: 1.5, unit: 'tbsp' },
      { item: 'mirin', amount: 1, unit: 'tbsp' },
      { item: 'furikake', amount: 1, unit: 'tbsp' },
      { item: 'sesameseed', amount: 1, unit: 'tsp' },
    ],
    steps: [
      'Whisk miso and mirin; brush over the protein.',
      'Air-fry at the shown temp — use the timer.',
      'Shower with furikake and sesame; 1 more minute to set.',
    ],
  },
  {
    id: 'g-harissa-honey',
    name: 'Harissa–Honey',
    mode: 'air-fryer',
    lane: 'Spicy',
    usesProtein: true,
    blurb: 'North-African chili paste with honey and lemon.',
    ingredients: [
      { item: 'harissa', amount: 1, unit: 'tbsp' },
      { item: 'honey', amount: 1, unit: 'tbsp' },
      { item: 'lemon', amount: 0.5, unit: 'each' },
      { item: 'garlic', amount: 1, unit: 'clove' },
    ],
    steps: [
      'Mix harissa, honey, lemon and minced garlic.',
      'Coat the protein and air-fry at the shown temp — use the timer.',
    ],
  },
  {
    id: 'g-scallion-sesame',
    name: 'Scallion–Sesame',
    mode: 'air-fryer',
    lane: 'Herby',
    usesProtein: true,
    blurb: 'A light soy-sesame glaze piled with scallion at the end.',
    ingredients: [
      { item: 'soy', amount: 1, unit: 'tbsp' },
      { item: 'sesameoil', amount: 1, unit: 'tsp' },
      { item: 'ricevinegar', amount: 1, unit: 'tsp' },
      { item: 'scallion', amount: 2, unit: 'stalks' },
      { item: 'sesameseed', amount: 1, unit: 'tsp' },
    ],
    steps: [
      'Whisk soy, sesame oil and rice vinegar.',
      'Glaze the protein and air-fry at the shown temp — use the timer.',
      'Top with sliced scallion and toasted sesame.',
    ],
  },
]

// rice-cooker builder. `usesProtein` bowls get the picked protein at render.
// `extra` can set the COSORI function (`cooker`) and a `setup` note (ratio·time);
// defaults to the White Rice program, which suits rice/jasmine dishes.
const rice = (id, name, lane, blurb, ingredients, steps, usesProtein = false, extra = {}) => ({
  id,
  name,
  mode: 'rice-cooker',
  lane,
  blurb,
  usesProtein,
  cooker: extra.cooker ?? 'White Rice',
  setup: extra.setup,
  ingredients,
  steps,
})

export const RICE = [
  // Comfort — protein-swappable bowls
  rice(
    'r-furikake-bowl',
    'Furikake Rice Bowl',
    'Comfort',
    'Your protein over rice with furikake and scallion.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'furikake', amount: 1, unit: 'tbsp' },
      { item: 'scallion', amount: 1, unit: 'stalk' },
      { item: 'soy', amount: 1, unit: 'tsp' },
    ],
    [
      'Rinse rice; cook with the protein set on top.',
      'Flake or fold the protein through and finish with furikake, scallion and soy.',
    ],
    true,
  ),
  rice(
    'r-ginger-rice',
    'Ginger Rice',
    'Comfort',
    'One-pot ginger-garlic rice built around your protein.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'stock', amount: 1.75, unit: 'cups' },
      { item: 'ginger', amount: 1, unit: 'tbsp' },
      { item: 'garlic', amount: 2, unit: 'cloves' },
      { item: 'scallion', amount: 1, unit: 'stalk' },
    ],
    [
      'Rub the protein with ginger and garlic.',
      'Cook over rinsed rice in stock; slice or fold in and top with scallion.',
    ],
    true,
  ),
  rice(
    'r-mushroom-shoyu',
    'Mushroom Shoyu Rice',
    'Comfort',
    'Umami shiitake rice seasoned with soy and mirin.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'mushroom', amount: 100, unit: 'g' },
      { item: 'soy', amount: 1, unit: 'tbsp' },
      { item: 'mirin', amount: 1, unit: 'tbsp' },
      { item: 'scallion', amount: 1, unit: 'stalk' },
    ],
    [
      'Slice shiitake over rinsed rice with soy and mirin.',
      'Cook, fold, and finish with scallion.',
    ],
  ),
  rice(
    'r-egg-scallion',
    'Egg & Scallion Rice',
    'Comfort',
    'Silky egg folded through hot rice with sesame.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'egg', amount: 2, unit: 'each' },
      { item: 'scallion', amount: 1, unit: 'stalk' },
      { item: 'sesameoil', amount: 1, unit: 'tsp' },
      { item: 'soy', amount: 1, unit: 'tsp' },
    ],
    [
      'Cook rice.',
      'Fold beaten egg into the hot rice on Keep-Warm; finish with scallion, sesame oil and soy.',
    ],
  ),
  // Spicy
  rice(
    'r-kimchi',
    'Kimchi Rice',
    'Spicy',
    'Tangy kimchi steamed straight into the rice.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.5, unit: 'cups' },
      { item: 'kimchi', amount: 0.75, unit: 'cup' },
      { item: 'scallion', amount: 1, unit: 'stalk' },
      { item: 'sesameoil', amount: 1, unit: 'tsp' },
      { item: 'soy', amount: 1, unit: 'tsp' },
    ],
    [
      'Chop kimchi and layer over rinsed rice with its juice.',
      'Cook, fold, finish with scallion, sesame oil and soy.',
    ],
  ),
  rice(
    'r-gochujang-rice',
    'Gochujang Rice',
    'Spicy',
    'Spicy-sweet gochujang glazing your protein over rice.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'gochujang', amount: 1, unit: 'tbsp' },
      { item: 'garlic', amount: 1, unit: 'clove' },
      { item: 'scallion', amount: 1, unit: 'stalk' },
    ],
    [
      'Toss the protein with gochujang and garlic; set over rinsed rice.',
      'Cook and fold with scallion.',
    ],
    true,
  ),
  rice(
    'r-chili-garlic-peas',
    'Chili-Garlic Rice & Peas',
    'Spicy',
    'Sriracha and garlic over your protein with sweet peas.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'peas', amount: 0.5, unit: 'cup' },
      { item: 'sriracha', amount: 1, unit: 'tbsp' },
      { item: 'garlic', amount: 1, unit: 'clove' },
    ],
    [
      'Toss the protein with sriracha and garlic.',
      'Add it and the peas for the last stretch of the cook; fold to combine.',
    ],
    true,
  ),
  rice(
    'r-harissa-chickpea',
    'Harissa Chickpea Rice',
    'Spicy',
    'Smoky harissa chickpeas with wilted spinach.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'stock', amount: 1.75, unit: 'cups' },
      { item: 'chickpea', amount: 1, unit: 'cup' },
      { item: 'harissa', amount: 1, unit: 'tbsp' },
      { item: 'spinach', amount: 1, unit: 'cup' },
      { item: 'garlic', amount: 1, unit: 'clove' },
    ],
    [
      'Stir harissa and garlic through rice and stock; add chickpeas.',
      'Fold spinach in at the end to wilt.',
    ],
  ),
  // Veggie
  rice(
    'r-edamame-sesame',
    'Edamame Sesame Rice',
    'Veggie',
    'Nutty sesame rice studded with green edamame.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'edamame', amount: 0.75, unit: 'cup' },
      { item: 'sesameseed', amount: 1, unit: 'tbsp' },
      { item: 'soy', amount: 1, unit: 'tbsp' },
      { item: 'scallion', amount: 1, unit: 'stalk' },
    ],
    ['Cook rice with edamame and soy.', 'Fold in sesame and scallion.'],
  ),
  rice(
    'r-spinach-garlic',
    'Spinach & Garlic Rice',
    'Veggie',
    'Garlicky green rice — spinach folded in at the finish.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'stock', amount: 1.75, unit: 'cups' },
      { item: 'spinach', amount: 1.5, unit: 'cups' },
      { item: 'garlic', amount: 2, unit: 'cloves' },
      { item: 'oil', amount: 1, unit: 'tsp' },
    ],
    ['Cook rice in stock with garlic.', 'Fold spinach through the hot rice to wilt; drizzle oil.'],
  ),
  rice(
    'r-carrot-pea-pilaf',
    'Carrot & Pea Pilaf',
    'Veggie',
    'Golden turmeric pilaf with sweet carrot and peas.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'stock', amount: 1.75, unit: 'cups' },
      { item: 'carrot', amount: 1, unit: 'each' },
      { item: 'peas', amount: 0.5, unit: 'cup' },
      { item: 'turmeric', amount: 0.5, unit: 'tsp' },
    ],
    ['Dice carrot; cook with rice, stock and turmeric.', 'Stir peas in for the last 8 minutes.'],
  ),
  rice(
    'r-mushroom-spinach',
    'Mushroom & Spinach Rice',
    'Veggie',
    'Earthy shiitake and spinach in soy-seasoned rice.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'mushroom', amount: 100, unit: 'g' },
      { item: 'spinach', amount: 1, unit: 'cup' },
      { item: 'garlic', amount: 1, unit: 'clove' },
      { item: 'soy', amount: 1, unit: 'tbsp' },
    ],
    ['Cook rice with sliced shiitake, garlic and soy.', 'Fold spinach in to wilt.'],
  ),
  rice(
    'r-ginger-pepper',
    'Ginger Pepper Rice',
    'Veggie',
    'Sweet peppers and your protein with ginger-soy.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'bellpepper', amount: 1, unit: 'each' },
      { item: 'soy', amount: 1, unit: 'tbsp' },
      { item: 'ginger', amount: 1, unit: 'tsp' },
    ],
    ['Cube the protein and pepper; layer over rice with soy and ginger.', 'Cook and fold gently.'],
    true,
  ),
  // Coconut & Curry
  rice(
    'r-coconut-jasmine',
    'Coconut Jasmine Rice',
    'Coconut & Curry',
    'Fragrant jasmine rice cooked in coconut milk.',
    [
      { item: 'jasmine', amount: 1.5, unit: 'cups' },
      { item: 'coconutmilk', amount: 1, unit: 'cup' },
      { item: 'water', amount: 0.75, unit: 'cup' },
      { item: 'salt', amount: 0.5, unit: 'tsp' },
    ],
    ['Rinse jasmine; cook with coconut milk, water and salt.', 'Fluff and rest 5 minutes.'],
  ),
  rice(
    'r-green-curry',
    'Green Curry Rice',
    'Coconut & Curry',
    'Thai green curry simmered into jasmine rice with your protein.',
    [
      { item: 'jasmine', amount: 1.5, unit: 'cups' },
      { item: 'coconutmilk', amount: 1, unit: 'cup' },
      { item: 'currypaste', amount: 1.5, unit: 'tbsp' },
      { item: 'basil', amount: 0.25, unit: 'cup' },
    ],
    [
      'Whisk curry paste into coconut milk; add the protein.',
      'Cook over jasmine rice; fold basil in at the end.',
    ],
    true,
  ),
  rice(
    'r-coconut-curry',
    'Coconut Curry Rice',
    'Coconut & Curry',
    'Sweet peppers and your protein in a coconut curry rice.',
    [
      { item: 'jasmine', amount: 1.5, unit: 'cups' },
      { item: 'coconutmilk', amount: 1, unit: 'cup' },
      { item: 'currypaste', amount: 1, unit: 'tbsp' },
      { item: 'bellpepper', amount: 1, unit: 'each' },
    ],
    [
      'Cook jasmine in coconut milk with curry paste and pepper.',
      'Add the protein for the last stretch of the cook.',
    ],
    true,
  ),
  rice(
    'r-peanut-rice',
    'Peanut Rice',
    'Coconut & Curry',
    'Creamy peanut-lime sauce over your protein and rice.',
    [
      { item: 'jasmine', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'peanut', amount: 2, unit: 'tbsp' },
      { item: 'soy', amount: 1, unit: 'tbsp' },
      { item: 'lime', amount: 0.5, unit: 'each' },
      { item: 'cilantro', amount: 0.25, unit: 'cup' },
    ],
    [
      'Cook jasmine with the protein.',
      'Loosen peanut butter with soy and lime; fold through with cilantro.',
    ],
    true,
  ),
  rice(
    'r-coconut-lentil',
    'Coconut Lentil Rice',
    'Coconut & Curry',
    'Golden red-lentil dal folded into coconut rice.',
    [
      { item: 'jasmine', amount: 1, unit: 'cup' },
      { item: 'lentil', amount: 0.5, unit: 'cup' },
      { item: 'coconutmilk', amount: 1, unit: 'cup' },
      { item: 'turmeric', amount: 0.5, unit: 'tsp' },
      { item: 'spinach', amount: 1, unit: 'cup' },
    ],
    ['Cook rice and lentils in coconut milk with turmeric.', 'Fold spinach in to wilt.'],
  ),
  rice(
    'r-curry-chickpea',
    'Curry Chickpea Rice',
    'Coconut & Curry',
    'Chickpeas and peas in a mild coconut-curry jasmine.',
    [
      { item: 'jasmine', amount: 1.5, unit: 'cups' },
      { item: 'coconutmilk', amount: 1, unit: 'cup' },
      { item: 'chickpea', amount: 1, unit: 'cup' },
      { item: 'currypaste', amount: 1, unit: 'tbsp' },
      { item: 'peas', amount: 0.5, unit: 'cup' },
    ],
    ['Cook jasmine in coconut milk with curry paste and chickpeas.', 'Stir peas in near the end.'],
  ),
  // Quick
  rice(
    'r-soy-butter',
    'Soy-Butter Rice',
    'Quick',
    'Three-ingredient magic: hot rice, soy, a knob of butter.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'soy', amount: 1, unit: 'tbsp' },
      { item: 'butter', amount: 1, unit: 'tbsp' },
      { item: 'scallion', amount: 1, unit: 'stalk' },
    ],
    ['Cook rice.', 'Fold in soy and butter; top with scallion.'],
  ),
  rice(
    'r-lemon-herb',
    'Lemon Herb Rice',
    'Quick',
    'Bright lemon-cilantro jasmine — a clean side.',
    [
      { item: 'jasmine', amount: 1.5, unit: 'cups' },
      { item: 'stock', amount: 1.75, unit: 'cups' },
      { item: 'lemon', amount: 0.5, unit: 'each' },
      { item: 'cilantro', amount: 0.25, unit: 'cup' },
    ],
    ['Cook jasmine in stock.', 'Fold in lemon juice and cilantro; fluff.'],
  ),
  rice(
    'r-sesame-soy',
    'Sesame Soy Rice',
    'Quick',
    'Toasty sesame-soy rice you can make on autopilot.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'sesameoil', amount: 1, unit: 'tsp' },
      { item: 'soy', amount: 1, unit: 'tbsp' },
      { item: 'sesameseed', amount: 1, unit: 'tsp' },
    ],
    ['Cook rice.', 'Fold in sesame oil and soy; top with sesame seeds.'],
  ),
  rice(
    'r-cilantro-lime',
    'Cilantro-Lime Rice',
    'Quick',
    'Zesty lime and cilantro jasmine — taqueria style.',
    [
      { item: 'jasmine', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'lime', amount: 1, unit: 'each' },
      { item: 'cilantro', amount: 0.33, unit: 'cup' },
      { item: 'salt', amount: 0.5, unit: 'tsp' },
    ],
    ['Cook jasmine with salt.', 'Fold in lime juice and cilantro; fluff.'],
  ),
  rice(
    'r-ginger-scallion',
    'Ginger Scallion Rice',
    'Quick',
    'The ginger-scallion oil trick, folded into rice.',
    [
      { item: 'rice', amount: 1.5, unit: 'cups' },
      { item: 'water', amount: 1.75, unit: 'cups' },
      { item: 'ginger', amount: 1, unit: 'tbsp' },
      { item: 'scallion', amount: 2, unit: 'stalks' },
      { item: 'sesameoil', amount: 1, unit: 'tsp' },
      { item: 'soy', amount: 1, unit: 'tsp' },
    ],
    ['Cook rice.', 'Warm ginger and scallion in sesame oil; fold through with soy.'],
  ),
  // Grains — showcase the COSORI Grains/Oatmeal programs (official ratios)
  rice(
    'r-quinoa-bowl',
    'Lemon Quinoa Bowl',
    'Grains',
    'Fluffy quinoa brightened with lemon and scallion.',
    [
      { item: 'quinoa', amount: 1, unit: 'cup' },
      { item: 'water', amount: 1.25, unit: 'cups' },
      { item: 'lemon', amount: 0.5, unit: 'each' },
      { item: 'scallion', amount: 1, unit: 'stalk' },
    ],
    [
      'Rinse quinoa to remove saponin.',
      'Run the Grains → Quinoa program; fold in lemon and scallion.',
    ],
    false,
    { cooker: 'Grains', setup: '1:1¼ · 37–47 min' },
  ),
  rice(
    'r-farro-mushroom',
    'Mushroom Farro',
    'Grains',
    'Chewy farro with shiitake and garlic — a hearty base.',
    [
      { item: 'farro', amount: 1, unit: 'cup' },
      { item: 'stock', amount: 1.25, unit: 'cups' },
      { item: 'mushroom', amount: 100, unit: 'g' },
      { item: 'garlic', amount: 1, unit: 'clove' },
    ],
    ['Add farro, stock, sliced shiitake and garlic.', 'Run the Grains → Farro program; fluff.'],
    false,
    { cooker: 'Grains', setup: '1:1¼ · 65–75 min' },
  ),
  rice(
    'r-maple-oats',
    'Maple Steel-Cut Oats',
    'Grains',
    'Creamy steel-cut oats with a swirl of maple.',
    [
      { item: 'oats', amount: 1, unit: 'cup' },
      { item: 'water', amount: 3, unit: 'cups' },
      { item: 'maple', amount: 1, unit: 'tbsp' },
    ],
    ['Add oats and water.', 'Run the Oatmeal program; stir in maple to finish.'],
    false,
    { cooker: 'Oatmeal', setup: '1:3 · 35–45 min' },
  ),
]

// Sides — not protein-swappable. Air-fryer sides carry their own temp/time/
// doneness/tip; steamed sides carry a COSORI Steam function.
export const SIDES = [
  {
    id: 's-broccoli-af',
    name: 'Crispy Roasted Broccoli',
    mode: 'air-fryer',
    lane: 'Veg',
    blurb: 'Charred, tender-crisp florets — the default green.',
    tempF: 400,
    cookSeconds: 600,
    doneness: 'tender-crisp, charred tips',
    tip: 'Toss in oil; single layer; shake halfway',
    ingredients: [
      { item: 'broccoli', amount: 3, unit: 'cups' },
      { item: 'oil', amount: 1, unit: 'tbsp' },
      { item: 'salt', amount: 0.5, unit: 'tsp' },
    ],
    steps: ['Toss florets in oil and salt.', 'Air-fry at 400°F, shaking halfway, until charred.'],
  },
  {
    id: 's-fries-af',
    name: 'Air-Fryer Fries',
    mode: 'air-fryer',
    lane: 'Potato',
    blurb: 'Frozen to golden with no thawing.',
    tempF: 400,
    cookSeconds: 780,
    doneness: 'golden & crisp',
    tip: 'Fill ≤ ¾ basket; shake every 5 min',
    ingredients: [{ item: 'fries', amount: 16, unit: 'oz' }],
    steps: [
      'No thaw — straight from frozen.',
      'Air-fry at 400°F, shaking every 5 min, until crisp.',
    ],
  },
  {
    id: 's-brussels-af',
    name: 'Charred Brussels Sprouts',
    mode: 'air-fryer',
    lane: 'Veg',
    blurb: 'Crispy leaves, caramelised cut sides.',
    tempF: 380,
    cookSeconds: 900,
    doneness: 'crispy, deeply browned',
    tip: 'Halve; cut-side down; oil',
    ingredients: [
      { item: 'brusselsprouts', amount: 2, unit: 'cups' },
      { item: 'oil', amount: 1, unit: 'tbsp' },
    ],
    steps: ['Halve sprouts and toss with oil.', 'Air-fry at 380°F cut-side down until browned.'],
  },
  {
    id: 's-asparagus-af',
    name: 'Blistered Asparagus',
    mode: 'air-fryer',
    lane: 'Veg',
    blurb: 'Five minutes to blistered and bright.',
    tempF: 400,
    cookSeconds: 300,
    doneness: 'blistered, still snappy',
    tip: 'Very fast — watch closely',
    ingredients: [
      { item: 'asparagus', amount: 1, unit: 'bunch' },
      { item: 'oil', amount: 1, unit: 'tsp' },
    ],
    steps: ['Trim ends; toss with oil.', 'Air-fry at 400°F ~5 min until blistered.'],
  },
  {
    id: 's-babypotato-af',
    name: 'Crispy Baby Potatoes',
    mode: 'air-fryer',
    lane: 'Potato',
    blurb: 'Crisp shell, fluffy centre.',
    tempF: 400,
    cookSeconds: 1080,
    doneness: 'crisp outside, fluffy in',
    tip: 'Halve; oil + salt; single layer',
    ingredients: [
      { item: 'potato', amount: 1, unit: 'lb' },
      { item: 'oil', amount: 1, unit: 'tbsp' },
      { item: 'salt', amount: 0.5, unit: 'tsp' },
    ],
    steps: ['Halve and toss with oil and salt.', 'Air-fry at 400°F, shaking, until crisp.'],
  },
  {
    id: 's-wings-af',
    name: 'Buffalo Wings',
    mode: 'air-fryer',
    lane: 'Wings',
    blurb: 'Craggy, crackly skin — the baking-powder trick.',
    tempF: 400,
    cookSeconds: 1200,
    doneness: '175–185°F, crisp skin',
    tip: 'Pat very dry; ½ tsp baking powder/lb; single layer',
    ingredients: [
      { item: 'wings', amount: 1, unit: 'lb' },
      { item: 'sriracha', amount: 2, unit: 'tbsp' },
      { item: 'butter', amount: 1, unit: 'tbsp' },
    ],
    steps: [
      'Pat wings very dry; dust with a little baking powder.',
      'Air-fry at 400°F, flipping halfway, to 175–185°F.',
      'Toss in melted butter and sriracha.',
    ],
  },
  {
    id: 's-greenbeans-steam',
    name: 'Steamed Green Beans',
    mode: 'rice-cooker',
    lane: 'Steam',
    blurb: 'Crisp-tender beans, straight over the rice.',
    cooker: 'Steam',
    setup: '212°F · 8–10 min',
    ingredients: [{ item: 'greenbeans', amount: 2, unit: 'cups' }],
    steps: ['Water to the STEAM line; beans in the basket.', 'Steam 8–10 min to crisp-tender.'],
  },
  {
    id: 's-broccoli-steam',
    name: 'Steamed Broccoli',
    mode: 'rice-cooker',
    lane: 'Steam',
    blurb: 'The hands-off green — finishes with the rice.',
    cooker: 'Steam',
    setup: '212°F · 5–10 min',
    ingredients: [{ item: 'broccoli', amount: 3, unit: 'cups' }],
    steps: [
      'Set the basket over water (or on the rice at ~30 min).',
      'Steam 5–10 min; check early.',
    ],
  },
]
